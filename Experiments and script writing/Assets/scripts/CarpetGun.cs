...
public class CarpetGun : MonoBehaviour {
        ...
        if (TimeElapsed >= DelayBetweenShots)
        {
            for (float x = -5; x <= 5; ++x)
                for (float y = Mathf.Abs(x) - 5; y <= 5 - Mathf.Abs(x); ++y)
                {
                    ...
                    Vector3 newDir = Vector3.RotateTowards(CurrentBullet.transform.forward, CurrentBullet.transform.right, 0.05f * x, 1.0f);
                    CurrentBullet.transform.rotation = Quaternion.LookRotation(newDir);
                    Vector3 newDIr = Vector3.RotateTowards(CurrentBullet.transform.forward, CurrentBullet.transform.up, 0.05f * y, 1.0f);
                    CurrentBullet.transform.rotation = Quaternion.LookRotation(newDIr);
